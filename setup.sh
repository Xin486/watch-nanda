...
if [ "$LOCAL_DB" = "1" ]; then
    ...
    if [ -n "$RUNNING" ]; then
        ...
        if [ -n "$PORT_BUSY" ]; then
            ...
            if [ "$DB_USER" != "root" ]; then
                docker exec -i -e MYSQL_PWD="$DB_PASSWORD" "$CONTAINER_NAME" mysql -uroot -e \
    ...
    if [ "$USE_DOCKER" = "1" ]; then
        ...
        for i in $(seq 1 60); do
            # 容器若已退出/处于重启循环，提前报错而不是干等
            STATE=$(docker inspect -f '{{.State.Status}}' "$CONTAINER_NAME" 2>/dev/null || echo "gone")
            if [ "$STATE" != "running" ]; then
                fail "MySQL 容器状态异常($STATE)，请检查: docker logs $CONTAINER_NAME"
            fi
            if docker exec -e MYSQL_PWD="$DB_PASSWORD" "$CONTAINER_NAME" mysql -u"$DB_USER" -N -e "SELECT 1" >/dev/null 2>&1; then
                READY=1; break
            fi
        ...
        if [ "$READY" != "1" ]; then
            warn "MySQL 最近日志如下（用于排查）："
            docker logs --tail 20 "$CONTAINER_NAME" 2>&1 | tail -n 20 || true
